...
export const redirectToAuthCodeUrl = async (event: RequestEvent) => {
  ...
  const authCodeUrlRequest = {
    ...
    scopes: ['api://<APP CLIENT ID>/.default'],
    state,
  };
    ...
    if (decodedState.csrfToken === csrfToken) {
      ...
      if (code) {
        const authCodeRequest = {
          ...
          clientInfo: event.cookies.get("clientInfo"),
          origin: event.cookies.get("origin"),
        };
        ...
        try {
          ...
          event.cookies.set("userId", tokenResponse?.account?.homeAccountId ?? '', cookiesConfig);
          event.cookies.set("username", tokenResponse?.account?.username ?? '', cookiesConfig);
