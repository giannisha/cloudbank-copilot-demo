...
function handleUserQuery(userQuery) {
    ...
    const azureOpenAIApiKey = config.openAiKey
...
window.onload = () => {
    ...
    // setTimeout(window.startSession, 6000);
}

window.startSession = () => {
    const cogSvcRegion = config.cogSvcRegion;
    ...
    speechSynthesisConfig.outputFormat = SpeechSDK.OutputFormat.Simple
    speechSynthesisConfig.speechOutputFormat = SpeechSDK.SpeechSynthesisOutputFormat.Audio16Khz16KbpsMonoSiren
    ...
    const iceServerUsername = config.iceServerUsername;
    const iceServerCredential = config.iceServerCredential;
...
window.microphone = () => {
    if (document.getElementById('microphone').className === 'stopMicrophone') {
        // Stop microphone
        document.getElementById('microphone').disabled = true
        speechRecognizer.stopContinuousRecognitionAsync(
            () => {
                document.getElementById('microphone').className = 'startMicrophone'
                document.getElementById('microphone').disabled = false
            }, (err) => {
    ...
    speechRecognizer.startContinuousRecognitionAsync(
        () => {
            document.getElementById('microphone').className = 'stopMicrophone'
            document.getElementById('microphone').disabled = false
        }, (err) => {
